...
measure(State) ->
    ...
    case hera_data:get(robot_pos, robot) of 
        [{_, _, _, [_OldX,_OldY, OldAngle, OldRoom]}] ->
            ...
            R = q2dcm(Xor),
            {Acc,Acclin,Gyro,Mag} = get_val_nav(R)
            ...
            case get_new_robot_pos(OldRoom) of
                no_intersection ->
                    ...
                    [{Xor1,Por1}] = kalman_orientation(Acc,Acclin,Gyro,Mag,R0,Xnew),
                    ...
                    Yaw = quat_to_yaw(normalize_quat(Xor1)), % Je dois check si je dois le normaliser
                    NewState = #{ 
                        t0   => T1,
                        x_pos => Xpred,
                        p_pos => Ppred,
                ...
                {Xout, Yout} ->
                    ...
                    Z = mat:matrix([[Xout],[Yout],[AxLin],[AyLin]]),
                    R = mat:diag([?VAR_S, ?VAR_S,?VAR_AZ,?VAR_AZ]),
...
calibrate() ->
    ...
    {M_x_List, M_y_List, M_z_List} = [pmod_nav:read(acc, [out_x_m, out_y_m, out_z_m]) || _ <- lists:seq(1, N)],

    {Gx0_pos, Gy0_pos, Gz0_pos} = [lists:sum([Y || [Y] <- List]) / N || List <- [G_x_List, G_y_List, G_z_List]],
    {M_x_List, M_y_List, M_z_List} = [lists:sum([Y || [Y] <- List]) / N || List <- [ACC_x_List, ACC_y_List, ACC_z_List]],
...
get_val_nav(R) ->
    ...
    [Mx, My, Mz] = pmod_nav:read(acc, [out_x_,m out_y_m, out_z_m]),
...
kalman_orientation(Acc,Acclin,Gyro,Mag,R0,Xnew) ->
    [_,_,[Accx],_,_,[Accy],_,_,[Accz]] = Xnew,
