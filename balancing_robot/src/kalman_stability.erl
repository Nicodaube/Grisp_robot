...
init(_Args) ->
    ...
    hera_data:store(frequency, self(), 1, {0, 0, 200.0, T0}),
    hera_data:store(delta_time, self(), 1, [0.0]),
    hera_data:store(k_stability_state, self(), 1, [T0, X0, P0]),    
    Gy0 = hera_data:get(gyroscope),
    hera_data:store(comp_filter_state, self(), 1, [Gy0, 0.0, 0.0]),
...
measure(State) ->
    ...
    {Tk, Xk, Pk} = hera_data:get(k_stability_state),
    {Gyk, Angle_Complem, Angle_Rate} = hera_data:get(comp_filter_state),

    {Tk_1, Dt} = get_delta_time(Tk),
    [Gyk_1, Axk_1, Azk_1] = hera_data:get(gyroscope),

    {X1, P1} = kalman_angle(Dt, Axk_1, Azk_1, Gyk_1, Gyk, Xk, Pk),
    {_, _, Mean_Freq} = hera_data:get(frequency),
    ...
    hera_data:store(delta_time, self(), Seq, [Dt]),
    hera_data:store(k_stability_state, self(), Seq, [Tk_1, X1, P1]),
    hera_data:store(comp_filter_state, self(), Seq, [Gyk, Angle_Complem_New, Angle_Rate_New]),

    NewState = State#{seq => Seq +1},
    {ok, [{Tk_1, X1, P1}, {Gyk_1, Angle_Complem_New, Angle_Rate_New}], kalman, self(), NewState}.
